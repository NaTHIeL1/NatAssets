...
namespace NATFrameWork.NatAsset.Runtime
{
    ...
    public abstract class BaseProvider : IReference
    {
        ...
        public static T Create<T>(string providerGUID, Priority priority) where T : BaseProvider, new()
        {
            T loadTask = ReferencePool.Get<T>();
            loadTask._providerGUID = providerGUID;
        ...
        protected bool CommonAssetTaskLogic(string assetName, TaskRunner taskRunner, Priority priority,
            out BaseTask baseTask)
        {
            CommonTaskLogic<AssetTask>(assetName, taskRunner, priority, out baseTask);
            return true;
        }
            ...
            out BaseTask baseTask)
        {
            ...
            else
            {
                ...
                {
                    CommonTaskLogic<WebBundleTask>(bundleName, taskRunner, priority, out baseTask);
                }
                else
                {
                    CommonTaskLogic<BundleTask>(bundleName, taskRunner, priority, out baseTask);
                }
                return true;
            }
        }

        protected void CommonTaskLogic<T>(string taskName, TaskRunner taskRunner, Priority priority,
            out BaseTask baseTask) where T : BaseTask, new()
        {
            if (taskRunner.TryGetValue(taskName, out BaseTask tempTask))
            {
                baseTask = (T) tempTask;
            ...
            {
                baseTask = BaseTask.CreateTask<T>(taskName, priority);
